		...
		case "audio":
			...
			if EnableLatencyDebug {
				if lastUserAudioTime != nil && !lastUserAudioTime.IsZero() {
					...
					log.Printf("[Latency] EL_IN audio at %v; no last TWILIO_IN timestamp recorded",
						now.Format(time.RFC3339Nano))
				}
			}

			// Decode ElevenLabs PCM16 16k
			...
			if err := twilioConn.WriteJSON(resp); err != nil {
				...
				log.Printf("[Latency] TWILIO_OUT media at %v (send duration: %s)",
					time.Now().Format(time.RFC3339Nano), time.Since(sendStart).String())
			}
...
// For now these are mocked – they don’t affect audio path
